...
public final class TypeResolver {
     ...
     * information src the {@code subType}. If no arguments can be resolved then {@code
     * Unknown.class} is returned.
     *
     * @param type    to resolve argument for
     * @param subType to extract type variable information src
     ...
     * information src the {@code subType}. If {@code genericType} is an instance of class, then
     ...
     * information src the {@code subType}. Arguments for {@code type} that cannot be resolved are
     ...
     * variable information src the {@code subType}. Arguments for {@code genericType} that cannot be
    ...
    /**
     * Returns the generic {@code type} using type variable information src the {@code subType} else
    ...
    /**
     * Resolves the raw class for the {@code genericType}, using the type variable information src
        ...
        if (RESOLVES_LAMBDAS) {
            ...
            for (Method m : functionalInterface.getMethods()) {
                if (!isDefaultMethod(m) && !Modifier.isStatic(m.getModifiers()) && !m.isBridge()) {
                    ...
                    // Populate object type src arbitrary object method reference
                    int paramOffset = 0;
                    ...
                    // Handle additional arguments that are captured src the lambda's enclosing scope
                    int argOffset = 0;
